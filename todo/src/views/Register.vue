<template>
              ...
              <form class="space-y-4 md:space-y-6" @submit.prevent="handleSubmit">
                <div class="text-sm font-light text-red-600 dark:text-gray-400" v-if="registrationError">Error: {{ registrationError }}</div>
                  ...
                  <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                      Already have an account? <RouterLink to="/login" class="font-medium text-primary-600 hover:underline dark:text-primary-500">Login here</RouterLink>
                  </p>
              </form>
          </div>
      </div>
  ...
  const registering = computed(() => store.getters['auth/registering']);
  const registrationErrorCode = computed(() => store.getters['auth/registrationErrorCode']);
  const registrationError = computed(() => store.getters['auth/registrationError']);
  
  const register = async (credentials) => {
    const success = await store.dispatch('auth/register', credentials);
    if (success) {
      ...
      store.dispatch('notification/addMessage', {
        id: Date.now(),
        type: 'error',
        text: store.getters['auth/registrationError'],
      });
    }
  };
